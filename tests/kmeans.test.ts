...
describe('K-Means Clustering', () => {
  test('should correctly cluster simple 2D data', () => {
    const data = [
      [1, 1], [1.5, 2], [1, 1.5],  // Cluster 1
      [5, 5], [5.5, 5.5], [5, 5.5], // Cluster 2
      [9, 9], [9.5, 9], [9, 9.5]    // Cluster 3
    ];
    ...
    const cluster1Points = result.find(c => 
      euclideanDistance(c.centroid, [1.17, 1.5]) < 1
    )?.points;
  ...
  test('should throw error for empty dataset', () => {
    expect(() => kMeans([], 3)).toThrow('Invalid number of clusters or empty dataset');
  });

  test('should throw error when k > dataset size', () => {
    const data = [[1, 1], [2, 2]];
    expect(() => kMeans(data, 3)).toThrow('Invalid number of clusters or empty dataset');
  });

  test('should throw error for inconsistent dimensions', () => {
    const data = [[1, 1], [2, 2, 3], [4, 4]];
    expect(() => kMeans(data, 2)).toThrow('All data points must have the same dimensions');
  });
  ...
  test('should produce stable results with different runs', () => {
    ...
    const uniqueSizes = new Set(clusterSizes.map(sizes => JSON.stringify(sizes)));
    expect(uniqueSizes.size).toBe(1);
  });

  test('should handle higher dimensional data', () => {
    const data = [
      [1, 1, 1], [1.5, 1.5, 1.5], [1, 1, 1.5],
  ...
  test('should respect tolerance parameter', () => {
    ...
    const resultStrict = kMeans(data, k, 100, strictTolerance);
    const resultLoose = kMeans(data, k, 100, looseTolerance);
